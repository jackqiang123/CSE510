...
public class BT  implements GlobalConst{
  ...
  public final static int keyCompare(KeyClass key1, KeyClass key2)
    throws KeyNotMatchException
    {
      ...
      else if ((key1 instanceof ShapeKey) && (key2 instanceof ShapeKey))
      {
    	  return (((ShapeKey)key1).getKey().x1 - ((ShapeKey)key2).getKey().x1);
      }
  ...
  protected final static int getKeyLength(KeyClass key) 
	...
	return  outstr.size();
      ...
      else if (key instanceof ShapeKey)
      {
    	  return 16;
      }
      else
      {
    	  ...
    	  throw new KeyNotMatchException(null, "key types l do not match"); 
      }
    }
  ...
  protected final static 
	...
	else if (keyType == AttrType.attrShape)
	{
		key = new ShapeKey(Convert.getShapeValue(offset, from));
	}
	else 
  ...
  protected final static byte[] getBytesFromEntry( KeyDataEntry entry ) 
	   ...
	   ConvertException
    {
      ...
      try
      {
    	  //System.out.println("bt ln 214 entry.key: " + entry.key + " and entry.data: " + entry.data);
        n=getKeyLength(entry.key);
        m=n;
        
        //System.out.println("bt ln 217 entry.key: " + entry.key + " and entry.data: " + entry.data);
	  ...
	  Convert.setIntValue( ((IntegerKey)entry.key).getKey().intValue(), 0, data);
        ...
        else if (entry.key instanceof ShapeKey)
        {
        	Convert.setShapeValue(((ShapeKey)entry.key).getKey(), 0, data);
        }
        else throw new KeyNotMatchException(null, "key types do not match");
