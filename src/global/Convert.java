 ...
 */
  ...
  /**
   * reads a shape from the given byte array at the specified position
   * convert it to a character
   * @param       data            a byte array
   * @param       position        the position in data[]
   * @exception   java.io.IOException I/O errors
   * @return      the character
   */
  public static Shape getShapeValue (int position, byte []data)
    throws java.io.IOException
    {
      InputStream in;
      DataInputStream instr;
      ...
      in = new ByteArrayInputStream(tmp);
      instr = new DataInputStream(in);
      int x1 = instr.readInt();
      int y1 = instr.readInt();
      int x2 = instr.readInt();
      int y2 = instr.readInt();
        ...
        throws java.io.IOException
 {
  ...
  public static void setShapeValue (Shape value, int position, byte []data)
		    throws java.io.IOException
		    {
		      /* creates a new data output stream to write data to
		       * underlying output stream
		      ...
		      // write the value to the output stream
		      outstr.writeInt(value.x1);  
		      outstr.writeInt(value.y1);  
		      outstr.writeInt(value.x2);  
		      outstr.writeInt(value.y2);  
		      // creates a byte array with this output stream size and the
		      // valid contents of the buffer have been copied into it
		      byte []B = ((ByteArrayOutputStream) out).toByteArray();
		      
		      // copies contents of this byte array into data[]
		      System.arraycopy (B, 0, data, position, 16);
